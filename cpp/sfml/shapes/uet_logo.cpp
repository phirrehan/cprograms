...
int main() {
  ...
  shape.setFillColor(sf::Color(15,7,101));
  shape.setOutlineColor(sf::Color(166,134,40));
  shape.setOutlineThickness(5.f);
  shape.setPosition({400.f, 300.f});

  // set points
  int   points  = 8 * 6; // change first number to get desired number of points
  float radius = 150.f,
  ...
  for (int i = 0; i < points; i++) {
    if ((i+1) % 6 < 3) {
      ...
      shape.setPoint(i, sf::Vector2f(radius * 0.75, sf::degrees(angle * i - offset)));
    }
  }
