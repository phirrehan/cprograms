...
int main() {
  ...
  // create window
  sf::RenderWindow window(sf::VideoMode({800,600}), "TicTacToe", sf::State::Windowed, settings);
  ...
  auto DrawWinningLine = [&] (float size, float width, float position_x, float position_y, float angle, float scale = 1.f) {
    ...
    line.setOrigin(line.getGeometricCenter());
    line.setFillColor(sf::Color(220, 175, 169));
    line.setPosition({position_x, position_y});
  ...
  auto DrawCursor = [&] (float size, float width, float position_x, float position_y) {
    ...
    cursor.setOutlineColor(sf::Color(220, 175, 169));
  ...
  while (window.isOpen()) {
    ...
    while (std::optional event = window.pollEvent()) {
      if (event->is<sf::Event::Closed>()) {
        ...
        if (winner == ' ')
        {
          ...
          if ((key->scancode == sf::Keyboard::Scancode::A || key->scancode == sf::Keyboard::Scancode::Left) && cursor_x > 0)
            cursor_x--;
          else if((key->scancode == sf::Keyboard::Scancode::D || key->scancode == sf::Keyboard::Scancode::Right) && cursor_x < 2)
            cursor_x++;
          else if((key->scancode == sf::Keyboard::Scancode::W || key->scancode == sf::Keyboard::Scancode::Up) && cursor_y > 0)
            cursor_y--;
          else if((key->scancode == sf::Keyboard::Scancode::S || key->scancode == sf::Keyboard::Scancode::Down) && cursor_y < 2)
            cursor_y++;

          // mark X or O (if empty)
          if (key->scancode == sf::Keyboard::Scancode::Space && state[cursor_y][cursor_x] == ' ')
          {
            if (is_X_turn)
        ...
        if ((key->scancode == sf::Keyboard::Scancode::R || key->scancode == sf::Keyboard::Scancode::Enter) && winner != ' ')
        {
          for (int j = 0; j < 3; j++)
            for (int i = 0; i < 3; i++)
              state[j][i] = ' ';
    ...
    // diagonal 1
    if(state[1][1] != ' ' && state[0][0] == state[1][1] && state[1][1] == state[2][2]) {
      DrawWinningLine(space*3, width/2, window_w/2, window_h/2, 45.f, 1.4f);
      winner = state[1][1];
    }
    // diagonal 2
    if(state[1][1] != ' ' && state[2][0] == state[1][1] && state[1][1] == state[0][2]) {
      DrawWinningLine(space*3, width/2, window_w/2, window_h/2, -45.f, 1.4f);
      winner = state[1][1];
    }
